...
export const notes = pgTable('notes', { id: serial('id').primaryKey(), ownerName: text('owner_name').notNull(), title: text('title').notNull(), content: text('content').notNull(), isShared: boolean('is_shared').notNull().default(false), createdAt: timestamp('created_at', { withTimezone: true }).notNull().defaultNow() })
...
export function toClientNote(note: Note) { return { id: String(note.id), title: note.title, content: note.content, owner: note.ownerName, shared: note.isShared, updated: note.createdAt.toISOString() } }
